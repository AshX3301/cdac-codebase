...
static long device_ioctl(struct file *file,unsigned int cmd,unsigned long arg){
	switch(cmd){
		case CMD_1:
			value = 0;
			pr_alert("This is command 1\n");
		break;
		case CMD_2:
			put_user(value,(int __user*)arg);
			pr_alert("This is command 2\n");
		break;
		case CMD_3:
			get_user(value,(int __user*)arg);
			pr_alert("This is command 3\n");
		break;
		default:
...
static int __init dev_entry(void){
	pr_alert("Char module initialized\n");
...
static void __exit dev_exit(void){
	...
	kfree(kernel_buffer);
	kernel_buffer = NULL;
	device_destroy(dev_file,dev);
	class_destroy(dev_file);
	cdev_del(&chdev);
	unregister_chrdev_region(dev,1);
        pr_alert("Char driver module detached\n");
